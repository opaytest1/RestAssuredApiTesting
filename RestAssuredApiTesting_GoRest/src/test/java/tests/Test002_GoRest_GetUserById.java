...
public class Test002_GoRest_GetUserById extends TestBase{
	...
	public void createUser() {
		logger.info("******** Started Test002_GoRest_GetUserById ********");
	...
	public void getUserById() {
		logger.info("Started getUserById() validation");
		int id = res.jsonPath().get("data.id");
		res = req.request(Method.GET, "/v1/users/" + id);
		if(res.statusCode() == 200) {
			logger.info("getUserById Success!");
			logger.info("Status code: " + res.statusCode());
			logger.info("Response body: " + res.getBody().asString());
		}
		logger.info("Finished getUserById() validation");
	}
	
	@AfterClass
	void tearDown() {
		logger.info("******** Stopped Test002_GoRest_GetUserById ********");
	}
}
